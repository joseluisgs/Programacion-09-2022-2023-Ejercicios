...
class ConcesionarioViewController : KoinComponent {
    ...
    @FXML
    fun initialize() {
        logger.debug { "Inicializando cochesDeViewController FXML" }
    ...
    private fun initBindings() {
        ...
        tableColumnModelo.cellValueFactory = PropertyValueFactory("marcaYModelo")
    ...
    private fun updatesFormulario(
        ...
        if (oldState.cocheSeleccionado != newState.cocheSeleccionado) {
            ...
            textMatricula.text = newState.cocheSeleccionado.matricula
            textCocheMarca.text = newState.cocheSeleccionado.marca
            textCocheModelo.text = newState.cocheSeleccionado.modelo
            textCocheTipoMotor.text = newState.cocheSeleccionado.tipoMotor
    ...
    private fun filterDataTable() {
        ...
        tableCoches.items = FXCollections.observableList(
            viewModel.cochesFilteredList(comboTipo.value, textBuscador.text.trim())
        )
    }
    ...
    private fun onImportarAction() {
        ...
        FileChooser().run {
            ...
            RoutesManager.activeStage.scene.cursor = WAIT
            viewModel.loadCochesFromJson(it)
                .onSuccess {
                    showAlertOperacion(
    ...
    fun onOnCloseAction() {
        ...
        Alert(AlertType.CONFIRMATION).apply {
            title = "Salir"
            headerText = "Salir de coches DAM"
            contentText = "¿Desea salir de coches DAM?"
        }.showAndWait().ifPresent { buttonType ->
            if (buttonType == ButtonType.OK) {
                Platform.exit()
            }
